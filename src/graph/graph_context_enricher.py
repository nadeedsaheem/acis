...
class GraphContextEnricher:
    ...
    def enrich(self, results, query=None):
        ...
        CALL (n) {
            OPTIONAL MATCH (n)-[:CONTAINS*1..6]->(topo) 
            WHERE labels(topo)[0] IN ['LUMP', 'SHELL', 'FACE', 'LOOP', 'COEDGE', 'EDGE', 'VERTEX']
            RETURN collect(DISTINCT 'HAS_' + labels(topo)[0]) AS topology
        }
        
        RETURN e.name AS name, e.entity_type AS entity_type,
               child_classes, parent_classes, methods, returned_by, 
               used_as_parameter, returns_types, owning_classes, enum_values,
               calls, called_by, topology
        """
        ...
        for r in results:
            ...
            if rels.get("topology"): new_r["relationships"]["topology"] = rels["topology"]
